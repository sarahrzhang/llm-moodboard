...
function centerPref(x: number) { return clamp01(1 - Math.abs(x - 0.5) * 2); }
...
function range(list: any[], key: string) {
  const vals = list.map(f => (f?.[key] ?? null)).filter((v: any) => typeof v === "number" && !Number.isNaN(v));
...
function scoreTrackNorm(fN: any, mode: Mode) {
  ...
  if (mode === "hype") {
    ...
    return 0.50 * energyN + 0.20 * danceN + 0.20 * tempoN + 0.10 * valenceN;
  }
  if (mode === "focus") {
    // instrumental, low speech; aim for mid energy/tempo to avoid hype & sleep
    return 0.50 * instrN + 0.25 * (1 - speechN) + 0.15 * centerPref(energyN) + 0.10 * centerPref(tempoN);
  }
  // chill: low energy/tempo, acoustic texture, pleasant valence
  return 0.45 * (1 - energyN) + 0.25 * acousticN + 0.20 * (1 - tempoN) + 0.10 * valenceN;
}
...
function tieBreak(a: any, b: any, mode: Mode) {
  const eps = 1e-3;
  if (mode === "hype") {
    const d1 = (b.featuresN.energyN - a.featuresN.energyN); if (Math.abs(d1) > eps) return d1;
    const d2 = (b.featuresN.tempoN - a.featuresN.tempoN);   if (Math.abs(d2) > eps) return d2;
    const d3 = (b.featuresN.danceN - a.featuresN.danceN);   if (Math.abs(d3) > eps) return d3;
  } else if (mode === "focus") {
    const d1 = (b.featuresN.instrN - a.featuresN.instrN);   if (Math.abs(d1) > eps) return d1;
    const d2 = ((1 - b.featuresN.speechN) - (1 - a.featuresN.speechN)); if (Math.abs(d2) > eps) return d2;
    const d3 = (centerPref(b.featuresN.energyN) - centerPref(a.featuresN.energyN)); if (Math.abs(d3) > eps) return d3;
  } else {
    const d1 = ((1 - b.featuresN.energyN) - (1 - a.featuresN.energyN)); if (Math.abs(d1) > eps) return d1;
    const d2 = (b.featuresN.acousticN - a.featuresN.acousticN); if (Math.abs(d2) > eps) return d2;
    const d3 = (b.featuresN.valenceN - a.featuresN.valenceN);   if (Math.abs(d3) > eps) return d3;
  }
...
async function fetchOnRepeatTracks(headers: Record<string,string>) {
  try {
    ...
    const onRepeat = all.find((p:any)=>p?.name==="On Repeat" && p?.owner?.id==="spotify");
    if (!onRepeat?.id) return [];
    let tracks: any[] = [];
    let tUrl: string | null = `https://api.spotify.com/v1/playlists/${onRepeat.id}/tracks?limit=100`;
    for (let i=0; i<4 && tUrl; i++) {
      const r = await fetch(tUrl, { headers, cache: "no-store" });
      if (!r.ok) break;
      const j = await readJSON(r);
      tracks = tracks.concat((j?.items ?? []).map((x:any)=>x?.track).filter(Boolean));
      tUrl = j?.next ?? null;
    }
...
async function fetchTopTracksShortTerm(headers: Record<string,string>) {
  const r = await fetch("https://api.spotify.com/v1/me/top/tracks?time_range=short_term&limit=20", { headers, cache: "no-store" });
  if (!r.ok) return [];
  const j = await readJSON(r);
  return j?.items ?? [];
}
...
export async function GET(req: Request) {
  ...
  const mode: Mode | null = (["hype","focus","chill"] as const).includes(modeParam as Mode) ? (modeParam as Mode) : null;
  ...
  if (!items.length) {
    return NextResponse.json({ error: "no_tracks", message: "Grant playlist-read-private and/or user-top-read; also ensure you have listening history." }, { status: 200 });
  }

  // audio-features
  const trackIds = items.map((t:any)=>t?.id).filter(Boolean);
  const afRes = trackIds.length
    ? await fetch(`https://api.spotify.com/v1/audio-features?ids=${trackIds.join(",")}`, { headers, cache: "no-store" })
    : null;
  const afJson = afRes && afRes.ok ? await readJSON(afRes) : { audio_features: [] };
  ...
  const stats = {
    valence_avg: featuresList.reduce((s,f:any)=>s+(f?.valence??0),0)/n,
    energy_avg: featuresList.reduce((s,f:any)=>s+(f?.energy??0),0)/n,
    danceability_avg: featuresList.reduce((s,f:any)=>s+(f?.danceability??0),0)/n,
    tempo_avg: featuresList.reduce((s,f:any)=>s+(f?.tempo??0),0)/n
  };

  // genres & top artists
  const artistIds = Array.from(new Set(items.flatMap((t:any)=>(t?.artists??[]).map((a:any)=>a?.id)).filter(Boolean)));
  let top_genres: string[] = [];
  if (artistIds.length) {
    const counts: Record<string,number> = {};
    for (let i=0;i<artistIds.length;i+=50) {
      const chunk = artistIds.slice(i,i+50);
      const r = await fetch(`https://api.spotify.com/v1/artists?ids=${chunk.join(",")}`, { headers, cache: "no-store" });
      if (!r.ok) continue;
      const j = await readJSON(r);
      for (const a of j?.artists ?? []) for (const g of a.genres ?? []) counts[g]=(counts[g]||0)+1;
    }
    top_genres = Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([g])=>g);
  }
  const top_artists = Array.from(new Set(items.flatMap((t:any)=>(t?.artists??[]).map((a:any)=>a?.name).filter(Boolean)))).slice(0,5);
  ...
  const tracks = items.map((t:any) => {
    ...
    const featuresN = {
      ...
      acousticN: minmax(f?.acousticness, R.acousticness.min, R.acousticness.max),
      instrN:    minmax(f?.instrumentalness, R.instrumentalness.min, R.instrumentalness.max),
  ...
  const debugOut = debug ? {
    hype:  sorted.slice(0,5).map(t=>({ name: t.name, score: t.scores.hype.toFixed(3) })),
    focus: sorted.slice(0,5).map(t=>({ name: t.name, score: t.scores.focus.toFixed(3) })),
    chill: sorted.slice(0,5).map(t=>({ name: t.name, score: t.scores.chill.toFixed(3) })),
  } : undefined;
